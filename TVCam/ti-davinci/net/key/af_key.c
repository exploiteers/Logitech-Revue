						...
						void **ext_hdrs)
{
   ...
   not true.
	...
	x->props.family = pfkey_sadb_addr2xfrm_addr(addr, &x->props.saddr);
	if (!x->props.family) {
		err = -EAFNOSUPPORT;
		goto out;
	}
	x->sel.sport = ((struct sockaddr_in *)(addr+1))->sin_port;
	if (x->sel.sport)
		x->sel.sport_mask = ~0;
	x->sel.proto = 	pfkey_proto_to_xfrm(addr->sadb_address_proto);

	addr = ext_hdrs[SADB_EXT_ADDRESS_DST-1];
	pfkey_sadb_addr2xfrm_addr(addr, &x->id.daddr);
	x->sel.dport = ((struct sockaddr_in *)(addr+1))->sin_port;
	if (x->sel.dport)
		x->sel.dport_mask = ~0;
	...
	if (ext_hdrs[SADB_EXT_ADDRESS_PROXY-1]) {
		addr = ext_hdrs[SADB_EXT_ADDRESS_PROXY-1];
...
static int pfkey_spdadd(struct sock *sk, struct sk_buff *skb, struct sadb_msg *hdr, void **ext_hdrs)
{
	...
	if (!pol->sadb_x_policy_dir || pol->sadb_x_policy_dir >= IPSEC_DIR_MAX)
		return -EINVAL;
#ifndef CONFIG_USE_POLICY_FWD
	if (pol->sadb_x_policy_dir == IPSEC_DIR_FWD)
		return -EINVAL;
#endif
			       ...
			       struct xfrm_user_migrate *m)
{
	...
	case AF_INET:
		...
		if ((sin+1)->sin_family != AF_INET)
			return -EINVAL;
		memcpy(&m->old_saddr.a4, &sin->sin_addr,
		       sizeof(struct in_addr));
		memcpy(&m->old_daddr.a4, &((sin+1)->sin_addr),
		       sizeof(struct in_addr));
		break;
#if defined(CONFIG_IPV6) || defined(CONFIG_IPV6_MODULE)
	case AF_INET6:
		...
		if ((sin6+1)->sin6_family != AF_INET6)
			return -EINVAL;
		memcpy(&m->old_saddr.a6, &sin6->sin6_addr,
		       sizeof(struct in6_addr));
		memcpy(&m->old_daddr.a6, &((sin6+1)->sin6_addr),
		       sizeof(struct in6_addr));
		break;
#endif
	default:
	...
	case AF_INET:
		...
		memcpy(&m->new_saddr.a4, &sin->sin_addr,
		       sizeof(struct in_addr));
		memcpy(&m->new_daddr.a4, &((sin+1)->sin_addr),
		       sizeof(struct in_addr));
		break;
#if defined(CONFIG_IPV6) || defined(CONFIG_IPV6_MODULE)
	case AF_INET6:
		sin6 = (struct sockaddr_in6 *)sa_new;
		memcpy(&m->new_saddr.a6, &sin6->sin6_addr,
		       sizeof(struct in6_addr));
		memcpy(&m->new_daddr.a6, &((sin6+1)->sin6_addr),
		       sizeof(struct in6_addr));
		break;
#endif
	default:
	...
	m->family = sa_new->sa_family;
	m->proto = req->sadb_x_ipsecrequest_proto;
	m->mode = req->sadb_x_ipsecrequest_mode - 1;
	m->reqid = req->sadb_x_ipsecrequest_reqid;
 ...
 *   - If using IPSEC_ULPROTO_ANY at SP's selector protocol of the message,
 *     all SP whose selector is matched without upper-layer protocol is to be
 *     updated.
 *   - If using IPSEC_PROTO_ANY at SA's IPsec protocol of the message, all
 *     matched SA is to be updated, for all IPsec protocol (e.g. AH/ESP).
 */
static int pfkey_migrate(struct sock *sk, struct sk_buff *skb, struct sadb_msg *hdr, void **ext_hdrs)
{
	...
	/* decode ISRs */
	if ((err = decode_ipsecrequest(pol, &m)) < 0) {
	...
	if (m.family == AF_INET6) {
		...
		printk("\tosrc = [%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x]\n",
		       NIP6(*((struct in6_addr *)&m.old_saddr.a6)));
		printk("\todst = [%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x]\n",
		       NIP6(*((struct in6_addr *)&m.old_daddr.a6)));
		printk("\tnsrc = [%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x]\n",
		       NIP6(*((struct in6_addr *)&m.new_saddr.a6)));
		printk("\tndst = [%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x]\n",
		       NIP6(*((struct in6_addr *)&m.new_daddr.a6)));
	}
#endif
#endif
...
#ifdef CONFIG_NET_KEY_MIGRATE
static int pfkey_send_migrate(struct xfrm_user_migrate *m,
