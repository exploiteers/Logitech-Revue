...
out2:
	rt->u.dst.lastuse = jiffies;
	rt->u.dst.__use++;
	// rt->u.dst.flow_cookie = atomic_read(&flow_cache_genid);
	return rt;
}
...
int ip6_route_add(struct in6_rtmsg *rtmsg, struct nlmsghdr *nlh, void *_rtattr,
		  struct netlink_skb_parms *req, u32 table_id)
{
	int err;
	struct rtmsg *r;
	struct rtattr **rta;
...
static void rt6_table_purge_dflt_routers(u32 table_id)
{
	struct rt6_info *rt;
	struct fib6_table *table;
...
{
	if (ifindex)
		rt6_table_purge_dflt_routers(RT6_TABLE_DFLT);
	else {
		int i;
...
#ifdef CONFIG_IPV6_STATISTICS
		IP6_INC_STATS(idev, IPSTATS_MIB_INADDRERRORS);
#else
