...
#ifdef CONFIG_IPV6_STATISTICS
		UDP6_INC_STATS_BH(idev, UDP_MIB_INERRORS);
#else
...
#ifdef CONFIG_IPV6_STATISTICS
	UDP6_INC_STATS_BH(idev, UDP_MIB_INDATAGRAMS);
#else
...
static int udpv6_rcv(struct sk_buff **pskb)
{
	...
	if (sk == NULL) {
		...
		if (skb_checksum_complete(skb))
			goto discard;
#ifdef CONFIG_IPV6_STATISTICS
		UDP6_INC_STATS_BH(idev, UDP_MIB_NOPORTS);
#else
...
out:
	...
	if (!err) {
		...
		/* XXX race? */
		idev = ((struct rt6_info *)dst)->rt6i_idev;
		UDP6_INC_STATS_USER(idev, UDP_MIB_OUTDATAGRAMS);
#else
