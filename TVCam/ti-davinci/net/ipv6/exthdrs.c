...
static int ipv6_dest_hao(struct sk_buff *skb, int optoff)
{
	...
	if (opt->dsthao) {
		LIMIT_NETDEBUG(KERN_INFO "hao duplicated\n");
		goto discard;
	}
	opt->dsthao = opt->dst;
	opt->dst = 0;
	...
	if (hao->length != 16) {
		LIMIT_NETDEBUG(
			KERN_INFO "hao invalid option length = %d\n", hao->length);
		goto discard;
	}

	if (!(ipv6_addr_type(&hao->addr) & IPV6_ADDR_UNICAST)) {
		LIMIT_NETDEBUG(
			KERN_INFO "hao is not an unicast addr: %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NIP6(hao->addr));
		goto discard;
	}
	...
	if (unlikely(ret < 0)) {
		LIMIT_NETDEBUG(
			KERN_INFO "unknown home address = %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NIP6(hao->addr));
		goto discard;
	}
...
looped_back:
	...
	if (hdr->type == IPV6_SRCRT_TYPE_2) {
		if (__xfrm6_rcv_one(skb,
				    ...
				    IPPROTO_ROUTING) < 0) {
			LIMIT_NETDEBUG(
				KERN_DEBUG "ipv6_rthdr_rcv: type 2: unknown home address = %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NIP6(skb->nh.ipv6h->daddr));
#ifdef CONFIG_IPV6_STATISTICS
			IP6_INC_STATS_BH(idev, IPSTATS_MIB_INADDRERRORS);
#else
			IP6_INC_STATS_BH(IPSTATS_MIB_INADDRERRORS);
		...
		if (!ipv6_chk_home_addr(addr)) {
			LIMIT_NETDEBUG(
				KERN_INFO "ipv6_rthdr_rcv: type 2: not a home address "
