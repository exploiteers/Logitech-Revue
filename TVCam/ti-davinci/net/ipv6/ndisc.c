...
#ifdef CONFIG_IPV6_NDISC_CHK_REDDST
				if (nd_opt->nd_opt_type == ND_OPT_REDIRECT_HDR &&
				    l < 8 + sizeof(struct ipv6hdr)) {
					ND_PRINTK2(KERN_WARNING
						   "%s(): too short redirected header (%d); ignored\n",
						   __FUNCTION__,
						   l);
					break;
				}
#endif /* CONFIG_IPV6_NDISC_CHK_REDDST */
	 	   ...
	 	   int router, int solicited, int override, int inc_opt) 
{
	...
	ifp = ipv6_get_ifaddr(solicited_addr, dev, 1);
 	if (ifp) {
		...
		if (ipv6_dev_get_saddr(dev, daddr, NULL, &tmpaddr) < 0)
			return;
		src_addr = &tmpaddr;
	}
...
static void ndisc_recv_ns(struct sk_buff *skb)
{
	...
	if ((ifp = ipv6_get_ifaddr(&msg->target, dev, 1)) != NULL) {
 	...
 	if (pneigh)
 		router = pneigh->flags & NTF_ROUTER;
 	else if (idev)
		...
		if (neigh->nud_state & NUD_FAILED)
			goto out;
               ...
               /* Don't update the neighbor cache entry on a proxy NA from
                  ourselves, since the proxied node is either off link or has
                  already sent a NA to us */
...
skip_defrtr:
	...
	if (in6_dev->cnf.accept_ra_pinfo && ndopts.nd_opts_pi) {
		...
		for (p = ndopts.nd_opts_pi;
			...
			addrconf_prefix_rcv(skb->dev, (u8*)p, (p->nd_opt_len) << 3, 0);
		}
	}
...
#ifdef CONFIG_IPV6_NDISC_CHK_REDDEST
	/*
	 * We believe that the redirect is bogus if the
	 * destination address of its carrier ipv6 header
	 * is different from the source address of the
	 * original ipv6 packet (in other words, the ipv6
	 * packet in the redirected header option),
	 * by definition.  Of course, we cannot check
	 * it if the redirected header option is absent.
	 *
	 * RFC2461 Section 4.5: Redirect Message Format
	 *
	 * Destination Address
	 *	The Source Address of the packet
	 *	that triggered the redirect.
	 *
	 * This option is disabled for now to pass the
	 * IPv6 Ready Logo Phase-2 Self Test with
	 * such bogus (and unrealistic) test cases.
	 * sigh... --yoshfuji
	 */
	if (ndopts.nd_opts_rh) {
		struct ipv6hdr *ip6h = (struct ipv6hdr *)(ndopts.nd_opts_rh + 8);
		if (ip6h->version != 6) {
			ND_PRINTK2(KERN_WARNING
				   "ICMPv6 Redirect: corrupted redirected header; ignored\n");
			in6_dev_put(in6_dev);
			return;
		}
		if (!ipv6_addr_equal(&skb->nh.ipv6h->daddr, &ip6h->saddr)) {
			ND_PRINTK2(KERN_WARNING
				   "ICMPv6 Redirect: incorrect destination for the original packet\n");
			in6_dev_put(in6_dev);
			return;
		}
	}
