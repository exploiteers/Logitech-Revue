...
#if 0
		/*
		 * This code is not correct; set_registers() invokes schedule(),
		 * while it is not allowed.
		 */
		struct dev_mc_list *mc;
		u8 mcfreg[8];

		memset(mcfreg, 0, sizeof(mcfreg));
		for (mc = net->mc_list; mc; mc = mc->next) {
			u8 crc = ether_crc_le(6, mc->dmi_addr) & 0x3f;
			mcfreg[crc >> 3] |= 1 << (crc & 0x7);
		}
		/* MCFCRC0 = 0x08 */
		set_registers(pegasus, MCFCRC0, sizeof(mcfreg), mcfreg);
#endif
		pegasus->eth_regs[EthCtrl0] &= ~RX_MULTICAST;
		pegasus->eth_regs[EthCtrl2] &= ~RX_PROMISCUOUS;
	}
