...
static inline void phy_on(void)
{
	...
	if (cpu_is_davinci_dm644x())
		...
		/* DM365 PHYCLKFREQ field [15:12] is set to 2 to
			 get clock from 24MHz xtal */
		__raw_writel(USBPHY_SESNDEN | USBPHY_VBDTCTEN | 0x2000,
								 IO_ADDRESS(USBPHY_CTL_PADDR));
...
static void davinci_source_power(struct musb *musb, int is_on, int immediate)
{
	...
	if (machine_is_davinci_evm() || machine_is_davinci_dm6467_evm()) {
		...
		/* modified EVM board switching VBUS with GPIO(2) not I2C
		 * NOTE:  PINMUX0.RGB888 (bit23) must be clear
		 */
		/*gpio_set_value(GPIO(6), is_on);*/
		gpio_set_value(GPIO(2), is_on);
		immediate = 1;
#else
		if (immediate)
...
#ifdef CONFIG_MACH_DM357_EVM
	gpio_direction_output(2, 0);
	temp1 = __raw_readl(gpioout);
	temp1 = temp1 | 0x00000004;
	__raw_writel(temp1,gpioout);
#endif
