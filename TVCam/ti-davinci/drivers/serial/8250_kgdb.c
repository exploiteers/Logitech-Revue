...
static unsigned int kgdb_ioread8(u8 mask)
{
	return ioread8(kgdb8250_addr + (mask << current_port->regshift));
}

static void kgdb_iowrite8(u8 val, u8 mask)
{
	iowrite8(val, kgdb8250_addr + (mask << current_port->regshift));
}

static unsigned int kgdb_ioread32(u8 mask)
{
	return ioread32(kgdb8250_addr + (mask << current_port->regshift));
}

static void kgdb_iowrite32(u8 val, u8 mask)
{
	iowrite32(val, kgdb8250_addr + (mask << current_port->regshift));
}

static unsigned int (*kgdb_ioread)(u8 mask) = kgdb_ioread8;
static void (*kgdb_iowrite)(u8 val, u8 mask) = kgdb_iowrite8;
...
static __init int kgdb_init_io(void)
{
	...
	case UPIO_MEM32:
		kgdb_ioread = kgdb_ioread32;
		kgdb_iowrite = kgdb_iowrite32;
		/* let the case falls through to initialize base address */
	case UPIO_MEM:
	case UPIO_TSI:
