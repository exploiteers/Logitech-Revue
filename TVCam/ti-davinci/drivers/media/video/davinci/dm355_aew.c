...
#include <asm/arch/vpss.h>	/* Local Definitions */
			...
			size_t size, loff_t *offset)
{
	...
	if (aew_dev_configptr->buffer_filled == 1) {
		...
		disable_irq(IRQ_DM3XX_VPSSINT4);
...
int __init aew_init(void)
{
	...
	result =
	    vpss_request_irq(VPSS_H3AINT, aew_isr, SA_SHIRQ, "dm644xh3a_aew",
...
void __exit aew_cleanup(void)
{
	...
	vpss_free_irq(VPSS_H3AINT, aew_dev_configptr);
