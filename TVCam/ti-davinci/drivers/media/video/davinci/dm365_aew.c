...
#include <asm/arch/dm365_a3_hw.h>
			...
			size_t size, loff_t *offset)
{
	...
	if (aew_dev_configptr->buffer_filled == 1) {
		...
		vpss_enable_irq(VPSS_AEW_INT, 0);
		...
		vpss_enable_irq(VPSS_AEW_INT, 1);
...
static irqreturn_t aew_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (vpss_clear_interrupt(VPSS_AEW_INT) < 0)
		return IRQ_RETVAL(IRQ_NONE);
...
int __init aew_init(void)
{
	...
	class_device_create(aew_class, NULL, dev, NULL, "dm365_aew");

	vpss_enable_clock(VPSS_H3A_CLOCK, 1);
	/* Set up the Interrupt handler for H3AINT interrupt */
	result =
	    vpss_request_irq(VPSS_AEW_INT, aew_isr, SA_SHIRQ, "dm365_h3a_aew",
...
void __exit aew_cleanup(void)
{
	...
	vpss_free_irq(VPSS_AEW_INT, aew_dev_configptr);
