...
static irqreturn_t af_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (vpss_clear_interrupt(VPSS_AF_INT) < 0)
		return IRQ_RETVAL(IRQ_NONE);
...
int __init af_init(void)
{
	...
	class_device_create(af_class, NULL, dev, NULL, "dm365_af");

	vpss_enable_clock(VPSS_H3A_CLOCK, 1);
	/* Set up the Interrupt handler for H3AINT interrupt */
	result =
	    vpss_request_irq(VPSS_AF_INT, af_isr, SA_SHIRQ, "dm365_h3a_af",
...
void __exit af_cleanup(void)
{
	...
	vpss_free_irq(VPSS_AF_INT, af_dev_configptr);
