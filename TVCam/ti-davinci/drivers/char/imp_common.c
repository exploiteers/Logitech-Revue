...
static int add_to_array(struct device *dev, struct imp_logical_channel *chan)
{
	...
	dev_dbg(dev, __func__ "E\n");

	/* locking the configuartion aaray */
	mutex_lock_interruptible(&imp_serializer_info.array_sem);
		     ...
		     struct imp_convert *convert)
{
	...
	if ((convert->out_buff1.size != 0)
		...
		if (chan->out_numbuf1s == 0) {
			...
			offset = convert->out_buff1.offset;
			status = 1;
		} else {
			/* memory mapped IO */
	...
	if ((convert->out_buff2.size != 0)
		...
		if (chan->out_numbuf2s == 0) {
			...
			status = 1;
			offset = convert->out_buff2.offset;
		} else {
			/* memory mapped IO */
	...
	if (prev)
		ret = vpss_request_irq(irq.sdram, imp_common_isr, SA_INTERRUPT,
