...
unsigned int mpic_get_one_irq(struct mpic *mpic, struct pt_regs *regs)
{
	...
	vec = src & MPIC_INFO(VECPRI_VECTOR_MASK);
#ifdef DEBUG_LOW
	DBG("%s: get_one_irq(): %d\n", mpic->name, src);
#endif
	if (unlikely(vec == mpic->spurious_vec)) {
		if (mpic->flags & MPIC_SPV_EOI)
			mpic_eoi(mpic);
		if (((mpic->flags & MPIC_REGSET_MASK) == MPIC_REGSET_TSI108) &&
	       		(src & TSI108_CPU_INTACK_LS_VECTOR_MSK))
				return NO_IRQ_IGNORE;
		return NO_IRQ;
	}
	return irq_linear_revmap(mpic->irqhost, vec);
}
