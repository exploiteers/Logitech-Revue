...
/* We have to do a dummy read on the AC97 controller for the RTC to work, WTF */
static void __devinit quirk_uli5455(struct pci_dev *dev)
{
	int i;
	u8 *dummy;
	...
	for (i = 0; i < DEVICE_COUNT_RESOURCE; i++) {
		if ((dev->resource[i].flags & IORESOURCE_MEM)) {
			dummy = ioremap(dev->resource[i].start, 0x4);
			if (dummy) {
				in_8(dummy);
				iounmap(dummy);
			}
...
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AL, 0x5455, quirk_uli5455);
