...
static void __devinit setup_APIC_timer(void)
{
	...
	/*
	 * The IO-APIC NMI depends on a regular timer on IRQ0 which
	 * is mirrored as the NMI interrupt. With HRT enabled the
	 * timer interrupt actually becomes a an hrtimer, so we
	 * can't allow hrtimers on the lapic, since it would
	 * likely run the IRQ0 on the lapic which breaks the NMI.
	 */
