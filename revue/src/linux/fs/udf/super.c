...
/******************************************************************
 ...
 Includes Intel Corporation's changes/modifications dated: 04/2009.
 Changed/modified portions - Copyright(c) 2009, Intel Corporation. 
 All rights reserved.
...
/*
 * The 2.6.23's implementation of udf_find_anchor() has an issue
 * which will print error messages during the mounting of blue-ray 
 * disc in some cases, so replace it with the 2.6.28's implementation
 * of udf_find_anchor() which is more robust. udf_check_anchor_block()
 * and udf_scan_anchors() are both called by udf_find_anchor().
 */
...
static void udf_find_anchor(struct super_block *sb)
{
	...
	lastblock = udf_scan_anchors(sb, sbi->s_lastblock);
	if (lastblock)
		goto check_anchor;
	...
	lastblock = udf_scan_anchors(sb,
				udf_variable_to_fixed(sbi->s_lastblock));
	if (lastblock)
		goto check_anchor;
