				      ...
				      const yaffs_ExtendedTags *tags)
{
	...
	struct gendisk *disk =
	    (yaffs_DeviceToContext(dev)->superBlock)->s_bdev->bd_disk;
	...
	ops.ooblen = (dev->param.inbandTags) ? 0 : YAFFS_PACKED_TAGS2_SIZE;
...
#endif
	preempt_disable();
	__disk_stat_add(disk, sectors[WRITE], dev->param.totalBytesPerChunk / 512);
	__disk_stat_inc(disk, ios[WRITE]);
	preempt_enable();
				       ...
				       __u8 *data, yaffs_ExtendedTags *tags)
{
	...
	if (dev->param.inbandTags || (data && !tags))
		...
		ops.mode = MTD_OOB_AUTO;
		ops.ooblen = YAFFS_PACKED_TAGS2_SIZE;
		ops.len = data ? dev->nDataBytesPerChunk : YAFFS_PACKED_TAGS2_SIZE;
		ops.ooboffs = 0;
		ops.datbuf = data;
	...
	if (dev->param.inbandTags) {
		...
		if (tags) {
			memcpy(&pt, yaffs_DeviceToContext(dev)->spareBuffer, YAFFS_PACKED_TAGS2_SIZE);
	...
	preempt_disable();
	__disk_stat_add(disk, sectors[READ], dev->param.totalBytesPerChunk / 512);
	__disk_stat_inc(disk, ios[READ]);
	preempt_enable();
