...
static int flash_ts_commit(struct flash_ts_priv *ts)
{
	 ...
	 */
	int max_iterations = (mtd->size / mtd->erasesize) * 2;
	size_t size = ALIGN(FLASH_TS_HDR_SIZE + ts->cache.len, mtd->writesize);
...
static int flash_ts_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
			  unsigned long arg)
{
