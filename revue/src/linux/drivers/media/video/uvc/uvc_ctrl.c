	...
	struct v4l2_queryctrl *v4l2_ctrl)
{
	...
	case V4L2_CTRL_TYPE_STRING:
		v4l2_ctrl->minimum = ctrl->info->size;
		v4l2_ctrl->maximum = ctrl->info->size;
		v4l2_ctrl->step = 1;
		ret = 0;
		goto out;
	...
	struct v4l2_ext_control *xctrl)
{
	...
	if (mapping->v4l2_type == V4L2_CTRL_TYPE_STRING) {
		if (copy_to_user(xctrl->string, uvc_ctrl_data(ctrl,
				 UVC_CTRL_DATA_CURRENT), ctrl->info->size))
			return -EFAULT;
	} else {
	...
	struct v4l2_ext_control *xctrl)
{
	...
	if (mapping->v4l2_type == V4L2_CTRL_TYPE_STRING) {
		if (copy_from_user(uvc_ctrl_data(ctrl, UVC_CTRL_DATA_CURRENT),
				   xctrl->string, ctrl->info->size))
			return -EFAULT;
	} else {
