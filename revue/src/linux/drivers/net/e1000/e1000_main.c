...
/*******************************************************************************
  Includes Intel Corporation's changes/modifications dated: 01/2009.
  Changed/modified portions - Copyright @ 2009, Intel Corporation. All rights reserved.
*******************************************************************************/
...
static unsigned long intel_ce_e1000_mdio_base_phy;	//Intel Media SOC GbE MDIO physical base address
void __iomem * intel_ce_e1000_mdio_base_virt;		//Intel Media SOC GbE MDIO virtual base address
void __iomem * gbe_config_base_virt;				//Intel Media SOC Configration Data virtual base address	
            ...
            const struct pci_device_id *ent)
{
	...
	if (adapter->hw.mac_type == e1000_cegbe)	{ 
		intel_ce_e1000_mdio_base_phy 	= pci_resource_start(pdev, BAR_1);
		intel_ce_e1000_mdio_base_virt = ioremap(intel_ce_e1000_mdio_base_phy, pci_resource_len(pdev, BAR_1));  
		if(!intel_ce_e1000_mdio_base_virt)
	...
	for (i = 0; i < 6; i++)
       ...
       if (adapter->hw.mac_type == e1000_cegbe)
       {
               for (i = 0; i < 32; i++)
          {
               adapter->hw.phy_addr = i;
              e1000_read_phy_reg(&adapter->hw, PHY_ID2 ,&tmp);
              if (tmp == 0 || tmp == 0xFF)
               {
...
err_sw_init:
err_mdio_ioremap:
	iounmap(intel_ce_e1000_mdio_base_virt);
		      ...
		      void *start, unsigned long len)
{
	...
	if (adapter->hw.mac_type == e1000_82545 ||
	    adapter->hw.mac_type == e1000_cegbe ||
