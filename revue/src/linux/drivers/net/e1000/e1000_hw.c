...
/*******************************************************************************
  ...
  Changed/modified portions - Copyright @ 2008, Intel Corporation. All rights reserved.
*******************************************************************************/
...
static int32_t
intel_ce_oem_phy_setup(struct e1000_hw *hw)
{
    int32_t ret_val;
    uint32_t ctrl_aux;
    switch(hw->phy_type)
	...
	case e1000_phy_rtl8201n:
            //Set RMII mode
            ctrl_aux = E1000_READ_REG(hw, CTL_AUX);
            ctrl_aux |= E1000_CTL_AUX_RMII;
            E1000_WRITE_REG(hw, CTL_AUX, ctrl_aux);
            E1000_WRITE_FLUSH(hw);
	    //Disable the J/K bits erquried for recieve
            ctrl_aux = E1000_READ_REG(hw, CTL_AUX);
            ctrl_aux |= 0x4;
            ctrl_aux &= ~0x2;
            E1000_WRITE_REG(hw, CTL_AUX, ctrl_aux);
...
e1000_copper_link_autoneg(struct e1000_hw *hw)
{
     ...
     * also limit to 10/100 on CE SoC with RTL8211B PHY (see erratum 214652)
     */
    if (hw->phy_type == e1000_phy_ife || hw->phy_type == e1000_phy_rtl8201n
#if defined(CONFIG_E1000_CE_DISABLE_GIGE)
        || (hw->mac_type == e1000_cegbe && hw->phy_type == e1000_phy_rtl8211b)
#endif
       )
...
e1000_setup_copper_link(struct e1000_hw *hw)
{
    ...
    if (hw->phy_type == e1000_phy_igp ||
        ...
        ret_val = intel_ce_oem_phy_setup(hw);
        if(ret_val)
		{
			printk(" intel_ce_oem_phy_setup failed!\n");
            return ret_val;
		}
	}
    ...
    if (hw->mac_type > e1000_82543) {
        ...
        if ( hw->mac_type == e1000_cegbe ) {
			mdic = ((reg_addr << E1000_MDIC_REG_SHIFT) |
                    ...
                    (INTEL_CE_E1000_MDIC_OP_READ) |
                    (INTEL_CE_E1000_MDIC_GO));
			...
			for(i = 0; i < 64; i++) {
				...
				if(!(mdic & INTEL_CE_E1000_MDIC_GO)) break;
			}			
            
			if(mdic & INTEL_CE_E1000_MDIC_GO) {
				DEBUGOUT("MDI Read did not complete\n");
				return -E1000_ERR_PHY;
			}
			
			mdic = readl(E1000_MDIO_STS);
			if(mdic & INTEL_CE_E1000_MDIC_READ_ERROR) {
