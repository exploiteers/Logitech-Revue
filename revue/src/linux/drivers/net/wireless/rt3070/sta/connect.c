	...
	IN MLME_QUEUE_ELEM * Elem) 
{
	...
	else if (INFRA_ON(pAd)) 
	{
          ...
          //
          // GoogleTV: Make sure we send the MLME request *only* if the
          // SSID has changed.
          if (!SSID_EQUAL(pAd->CommonCfg.Ssid, pAd->CommonCfg.SsidLen,
                          pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen)) {
            //
            // Different SSID means not Roaming case, so we let
            // LinkDown() to Indicate a disconnect event.
            //
            pAd->MlmeAux.CurrReqIsFromNdis = TRUE;
            // case 2. active INFRA association existent roaming is
            //    done within miniport driver, nothing to do with
            //    configuration utility. so upon a new
            //    SET(OID_802_11_SSID) is received, we just
            //    disassociate with the current associated AP, then
            //    perform a new association with this new SSID, no
            //    matter the new/old SSID are the same or not.
...
/* No need to perform this routine when ATE is running. */
	if (ATE_ON(pAd))
        ...
        // GoogleTV: only change the BSSID if the new value is different
        // from the old value
        if (RTMPCompareMemory(pAd->MlmeAux.Bssid, pOidBssid, MAC_ADDR_LEN) == 0)
          return;
