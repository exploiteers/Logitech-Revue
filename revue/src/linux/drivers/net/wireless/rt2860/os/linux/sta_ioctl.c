		      ...
		      struct sockaddr *ap_addr, char *extra)
{
	...
	if(!RTMP_TEST_FLAG(pAdapter, fRTMP_ADAPTER_INTERRUPT_IN_USE))
	{
        ...
        // GoogleTV: if this configuration request has already been
        // made, ignore.
        if (!memcmp(ap_addr->sa_data, pAdapter->MlmeAux.Bssid, MAC_ADDR_LEN)) {
          return 0;
        }
		...
		if ((OPSTATUS_TEST_FLAG(pAdapter, fOP_STATUS_MEDIA_STATE_CONNECTED)) &&
			((pAdapter->StaCfg.AuthMode == Ndis802_11AuthModeWPA) ||
                        (pAdapter->StaCfg.AuthMode == Ndis802_11AuthModeWPAPSK) ||
                         // GoogleTV: this applies to WPA2 as well
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	switch (param->flags & IW_AUTH_INDEX) {
    	...
    	case IW_AUTH_CIPHER_PAIRWISE:
            ...
            {
                // GoogleTV: for WEP, set up AutoSwitch mode, which
                // accomodates both WEP auth modes: Open and Shared
	...
	IN	INT					cmd)
{
	...
	// GoogleTV: added for wpa-supplicant compatibility
        case SIOCSIWPRIV: {
          extern INT extra_priv_ext(IN struct net_device *dev,
                                    IN struct iwreq * wrq);
          Status = extra_priv_ext(net_dev, wrq);
        } break;
