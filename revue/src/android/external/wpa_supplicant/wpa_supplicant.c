...
void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if ((wpa_s->scan_res_tried == 0 && wpa_s->conf->ap_scan == 1) || wpa_s->no_scan==1) {
			      ...
			      struct wpa_ssid *ssid)
{
    ...
    if (ssid->auth_alg) {
        ...
        if (ssid->auth_alg & WPA_AUTH_ALG_OPEN) {
            wpa_printf(MSG_DEBUG, "Automatic auth_alg selection: **** OPEN selected ****");
            algs |= AUTH_ALG_OPEN_SYSTEM;
        }
        if (ssid->auth_alg & WPA_AUTH_ALG_SHARED) {
            wpa_printf(MSG_DEBUG, "Automatic auth_alg selection: **** SHARED selected ****");
            algs |= AUTH_ALG_SHARED_KEY;
        }
        if (ssid->auth_alg & WPA_AUTH_ALG_LEAP) {
            wpa_printf(MSG_DEBUG, "Automatic auth_alg selection: **** LEAP selected ****");
            algs |= AUTH_ALG_LEAP;
        }
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		...
		if (assoc_failed) {
			...
			/* give IBSS a bit more time */
                        // GoogleTV: changed from 10 to 15 seconds, to
                        // allow extra time for slow RALink driver
 			timeout = ssid->mode ? 20 : 15;
		}
...
#define SCAN_AP_LIMIT 128
	...
	if(wpa_s->no_scan) {
		...
		wpa_s->no_scan = 0;
		wpa_printf(MSG_DEBUG, "Using the previous scan reults (as per no_scan flag)");
		return 0;
	}
