...
/*
 * This function checks if the current_ssid is dealing with WEP protocol
 * and it has tried SHARED mode & failed.
 * returns one if the above is true, else returns zero
 */
static int wpa_supplicant_check_for_wep(struct wpa_supplicant *wpa_s) 
{
    int i;

    /* 
     * here, check if (a) key_mgmt is set to NONE and
     * (b) there is at least one non-zero length key.
     * This concludes we are handling a WEP situation.
     */
    if (wpa_s->current_ssid->key_mgmt & WPA_KEY_MGMT_NONE) {
        for (i = 0; i < NUM_WEP_KEYS; i++) {
            if (wpa_s->current_ssid->wep_key_len[i]) {
                /*
                 * further check if the current WEP mode is OPEN
                 * otherwise, we don't have any further strategies
                 */
...
void wpa_driver_wext_auth_failure_event_handler(struct wpa_supplicant *wpa_s) 
{
    const u8 *bssid = wpa_s->bssid;
    struct wpa_ssid* ssid = wpa_s->current_ssid;

    if(ssid == NULL || wpa_s->scan_results == NULL) {
        wpa_printf(MSG_DEBUG, "%s: ignoring Authentication.Failure event ",__func__);
        return;
    }
    ...
    wpa_msg(wpa_s, MSG_INFO, "Authentication with " MACSTR " failed.",MAC2STR(bssid));

    if(wpa_supplicant_check_for_wep(wpa_s)) {
        wpa_printf(MSG_DEBUG, "%s: recognized as WEP (open) case",__func__);
        ...
        wpa_printf(MSG_DEBUG, "%s: Ignoring event - non-wep or wep (shared) case",__func__);
        return;
    }
    return;
}
