...
struct PayloadContext {
    ...
    // fragmented packet
    uint8_t *fragmented_data;
    int fragmented_length;
#ifdef DEBUG
...
{
    uint8_t nal = buf[0];
    uint8_t type = (nal & H264_NAL_TYPE_MASK);
    int result= 0;
    uint8_t start_sequence[]= {0, 0, 1};
    ...
    case 28:                   // FU-A (fragmented nal)
        ...
        len--;                  // skip the fu_indicator
        {
            ...
            uint8_t nal_type = (fu_header & H264_NAL_TYPE_MASK);
...
#endif
            if(start_bit) {
                // free any previous data, before starting a new packet....
                if (data->fragmented_data)
                {
                    av_free(data->fragmented_data);
                    data->fragmented_data=NULL;
                    data->fragmented_length = 0;
                }
                 // copy in the start sequence, and the reconstructed nal....
                data->fragmented_data = av_mallocz(sizeof(start_sequence)+sizeof(nal)+len);
                memcpy(data->fragmented_data, start_sequence, sizeof(start_sequence));
                data->fragmented_data[sizeof(start_sequence)]= reconstructed_nal;
                memcpy(data->fragmented_data+sizeof(start_sequence)+sizeof(nal), buf, len);
                data->fragmented_length = sizeof(start_sequence)+sizeof(nal)+len;
                result = -1;
            }
            else
            {
                // Check if start sequence bits are already copied, if not packet containing start bit may be lost, skip data....
                if (data->fragmented_data && (data->fragmented_length >= (sizeof(start_sequence) + sizeof(nal))) &&
                    !memcmp(data->fragmented_data, start_sequence, sizeof(start_sequence)))
                {
                    data->fragmented_data = av_realloc(data->fragmented_data, data->fragmented_length+len);
                    memcpy(data->fragmented_data+data->fragmented_length, buf, len);
                    data->fragmented_length = data->fragmented_length+len;

                    if(end_bit)
                    {
                        av_new_packet(pkt, data->fragmented_length);
                        memcpy(pkt->data, data->fragmented_data, data->fragmented_length);
                        av_free(data->fragmented_data);
                        data->fragmented_data=NULL;
                        data->fragmented_length = 0;
                    }
...
void rtp_alloc_h264_dynamic_handler(RTPDynamicProtocolHandler **handler, RTPDemuxContext **demuxer)
{
    if(!handler || !demuxer)
        return;

    *handler = &ff_h264_dynamic_handler;
    *demuxer = av_mallocz(sizeof(RTPDemuxContext));
    if(*demuxer)
        (*demuxer)->dynamic_protocol_context = (*handler)->open();
}

void rtp_free_h264_dynamic_handler(RTPDynamicProtocolHandler **handler, RTPDemuxContext **demuxer)
{
    if(!handler || !demuxer)
        return;
    ...
    if(*demuxer)
    {
        if(*handler)
        {
            (*handler)->close((*demuxer)->dynamic_protocol_context);
            *handler = NULL;
        }
