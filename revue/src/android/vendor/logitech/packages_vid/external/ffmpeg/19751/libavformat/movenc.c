...
static int mov_write_video_tag(ByteIOContext *pb, MOVTrack *track)
{
    ...
    else if((track->enc->codec_id == CODEC_ID_H264) || (track->enc->codec_id == CODEC_ID_FFH264))
...
static void mov_write_uuidprof_tag(ByteIOContext *pb, AVFormatContext *s)
{
    ...
    if ((VideoCodec->codec_id == CODEC_ID_H264) ||(VideoCodec->codec_id == CODEC_ID_FFH264)){
        put_tag(pb, "avc1");
        put_be16(pb, 0x014D);
        put_be16(pb, 0x0015);
    } else {
...
static int mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if ((enc->codec_id==CODEC_ID_FFH264 || enc->codec_id == CODEC_ID_H264) && trk->vosLen > 0 && *(uint8_t *)trk->vosData != 1) {
