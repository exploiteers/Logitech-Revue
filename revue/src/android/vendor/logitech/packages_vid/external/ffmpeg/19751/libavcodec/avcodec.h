...
// KLE - fixed for Canmore, since __INT64_C is not defined on Canore.
...
#define FF_LEVEL_UNKNOWN -99
     ...
     /**
     * the minimum number of I-blocks per frame when using CODEC_FLAG2_NO_I_FRAMES.
     * - encoding: set by user.
     * - decoding: unused
     */
     int i_refresh_rate;
    ...
    /**
     * the lambda factor for age distortion decision when using CODEC_FLAG2_NO_I_FRAMES.
     * - encoding: set by user.
     * - decoding: unused
     */
     int pqlambda;
...
void avcodec_reset_rate_control(AVCodecContext *s);
void avcodec_copy_reference_frame(AVCodecContext *s, uint8_t *dst);
...
/*
#include "dsputil.h" // for DECLARE_ALIGNED
#define PHASE_BITS 4
...
typedef struct ImgReSampleContext {
    ...
    int h_incr, v_incr;
    DECLARE_ALIGNED_8(int16_t, h_filters[NB_PHASES][NB_TAPS]); // horizontal
    DECLARE_ALIGNED_8(int16_t, v_filters[NB_PHASES][NB_TAPS]); // vertical
    uint8_t *line_buf;
} ImgReSampleContext;
*/
...
/**
 * deprecated in ffmpeg, we use it still for upsampling
 */
 ...
 * deprecated in ffmpeg, we use it still for upsampling
 */
void img_build_filter(short *filter, double factor, int tap_count, int phase_count, int scale, int type);
