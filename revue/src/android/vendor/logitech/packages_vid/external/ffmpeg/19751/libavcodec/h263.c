...
int mpeg4_get_es_configuration (uint8_t *config,size_t config_size,int profile,int width,int height,int fps){
    PutBitContext pb;

    if (!config || !config_size)
        return -1;

    init_put_bits(&pb, config, config_size);

    put_bits(&pb,16,0);
    put_bits(&pb,16,VOS_STARTCODE);
    put_bits(&pb,8,profile);									// profile and level

    put_bits(&pb,16,0);
    put_bits(&pb,16,VISUAL_OBJ_STARTCODE);
    put_bits(&pb,1, 1 );										// visual object version identification availability
    put_bits(&pb,4,((profile>>4)==0x0F)?5:1);					// version number visual object
    put_bits(&pb,3,1);											// priority
    put_bits(&pb,4,1);											// visual object type == video object
    put_bits(&pb,1,0);											// video signal type

    ff_mpeg4_stuffing(&pb);

    put_bits(&pb,16,0);
    put_bits(&pb,16,0x100);

    put_bits(&pb,16,0);
    put_bits(&pb,16,0x120);
    put_bits(&pb,1,0);											// random access vol
    put_bits(&pb,8,SIMPLE_VO_TYPE);	        					// video obj type indication

    put_bits(&pb,1,1);											// video object version identification availability
    put_bits(&pb,4,1);											// version number video object
    put_bits(&pb,3,1);											// priority
    put_bits(&pb,4,1);											// aspect ratio info
    put_bits(&pb,1,1);											// vol control parameters = yes
    put_bits(&pb,2,1);											// chroma format YUV 420/YV12
    put_bits(&pb,1,1);											// low delay
    ...
    put_bits(&pb,1,1);											// marker bit
    put_bits(&pb,13,width);										// vol width
    put_bits(&pb,1,1);											// marker bit
    put_bits(&pb,13,height);									// vol height
    put_bits(&pb,1,1);											// marker bit
    put_bits(&pb,1,0);											// interlaced video = no
    put_bits(&pb,1,1);											// obmc disable
    put_bits(&pb,1,0);											// sprite enable
    put_bits(&pb,1,0);											// not_8_bit == false
    put_bits(&pb,1,0);											// quant type = (0=h263 style)
    put_bits(&pb,1,1);											// complexity estimation disable
    put_bits(&pb,1,0);											// resync marker disable
    put_bits(&pb,1,0);
