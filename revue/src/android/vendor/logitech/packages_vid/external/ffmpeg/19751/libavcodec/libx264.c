...
#include "avcodec.h"
#include "C:/MinGW/1.0/local/include/x264.h"
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
encode_nals(AVCodecContext *ctx, uint8_t *buf, int size, x264_nal_t *nals, int nnal, int skip_sei)
{
    ...
    for(i = 0; i < nnal; i++){
        ...
        s = x264_nal_encode(p, &size, 1, nals + i);
         ...
         {
av_log(NULL, AV_LOG_DEBUG, "sending encoded nal %d: %d out of %d\n", size, i+1, nnal);
                if ((i+1==nnal) || (s<0))
                        avctx->rtp_callback(avctx, p, size, 1);
                else
                        avctx->rtp_callback(avctx, p, size, 0);
         }
...
X264_frame(AVCodecContext *ctx, uint8_t *buf, int bufsize, void *data)
{
    ...
    if (ctx->frame_number == 0) {
        ...
        ctx->extradata = av_malloc(s);
        ctx->extradata_size = encode_nals(ctx, ctx->extradata, s, nal, nnal);
    }
    ...
    if (frame) {
        ...
        if ((frame->key_frame==1) && (frame->pict_type == FF_I_TYPE))
...
X264_init(AVCodecContext *avctx)
{
    ...
    x4->params.i_slice_max = avctx->rtp_payload_size;
