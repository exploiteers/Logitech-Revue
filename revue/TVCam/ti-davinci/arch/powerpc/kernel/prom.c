...
static inline void identical_pvr_fixup(unsigned long node, const char *uname)
{
	unsigned int pvr;
...
#else
#define identical_pvr_fixup(node, uname) do { } while(0)
#endif
					  ...
					  void *data)
{
	...
	if (found) {
		...
		identical_pvr_fixup(node, uname);
	}
