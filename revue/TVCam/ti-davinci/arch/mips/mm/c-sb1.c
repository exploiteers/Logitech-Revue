...
static void sb1_flush_cache_data_page(unsigned long addr)
{
	if (in_atomic())
		...
		sb1_on_each_cpu(sb1_flush_cache_data_page_ipi, (void *) addr, 1, 1);
}
#else
static void local_sb1_flush_cache_data_page(void *info)
{
	unsigned long start = (unsigned long)info;
	__sb1_writeback_inv_dcache_range(start, start + PAGE_SIZE);
}
	...
	struct page *page)
{
	...
	args.vma = vma;
	args.page = page;
	sb1_on_each_cpu(sb1_flush_icache_page_ipi, (void *) &args, 1, 1);
}
#else
