...
//    printk(KERN_INFO "<%s> %d, \"%s\"\n", __FUNCTION__, channel, output);
    ...
    /* adv:cdc moved here for reference in "davinci_enc_set_mode_platform" 
        as we are already set to this output anyhow 
      */
    ...
    /* adv:cdc Moved the strcpy up as "davinci_enc_set_mode_platform" uses current_output */
	//if (0 == err)
	//	strcpy(enc_dev[channel].current_output, output);
...
#if 0
    /* adv:cdc Some debug tracing information */
    printk(KERN_INFO "<%s> %d,\n", __FUNCTION__, channel);
    printk(KERN_INFO "    signed char *name);           = \"%s\"\n", mode_info->name);         
    printk(KERN_INFO "    signed int std);              = %d\n", mode_info->std);            
    printk(KERN_INFO "    enum vid_enc_if_type if_type);= %d\n", mode_info->if_type);
    printk(KERN_INFO "    signed int interlaced);       = %d\n", mode_info->interlaced);     
    printk(KERN_INFO "    signed int xres);             = %d\n", mode_info->xres);           
    printk(KERN_INFO "    signed int yres);             = %d\n", mode_info->yres);           
    printk(KERN_INFO "    struct vid_enc_fract fps);    = %d/%d\n", mode_info->fps.numerator ,mode_info->fps.denominator);    
    printk(KERN_INFO "    signed int left_margin);      = %d\n", mode_info->left_margin);    
    printk(KERN_INFO "    signed int right_margin);     = %d\n", mode_info->right_margin);   
    printk(KERN_INFO "    signed int upper_margin);     = %d\n", mode_info->upper_margin);   
    printk(KERN_INFO "    signed int lower_margin);     = %d\n", mode_info->lower_margin);   
    printk(KERN_INFO "    signed int hsync_len);        = %d\n", mode_info->hsync_len);      
    printk(KERN_INFO "    signed int vsync_len);        = %d\n", mode_info->vsync_len);      
    printk(KERN_INFO "    signed int flags);            = %d\n", mode_info->flags);          
#endif
