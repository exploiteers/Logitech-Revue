...
static int tvp7002_initialize(void *dec, int flag)
{
	...
	if (tvp7002_channel_info[ch_id].i2c_dev.i2c_registration & 0x01) {
		printk(KERN_ERR "tvp7002 driver is already initialized..\n");
		err = -EINVAL;
		return err;
	}
	...
	if (err) {
		printk(KERN_ERR "Failed to register TVP7002 I2C client.\n");
		return -EINVAL;
	}
	...
	if (err) {
		dev_err(tvp7002_i2c_dev[ch_id], "Failed to set mux.\n");
		return -EINVAL;
	}
...
#ifdef DEBUG
static void tvp7002_dump_register(int ch_id)
{
	int i = 0, err;
	u8 val;
...
#ifdef DEBUG
	tvp7002_dump_register(ch_id);
#endif
	dev_notice(tvp7002_i2c_dev[ch_id],
