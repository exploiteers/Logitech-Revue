...
/*!
 * DMA Write callback is called by the SDMA controller after it has sent out
 * all the data from the user buffer. This function updates the xmit buffer
 * pointers.
 *
 * @param   arg   driver private data
...
/*!
 * The read DMA callback, this method is called when the DMA buffer
 * has received its data. This functions copies the data to the tty
 * buffer and updates the tty buffer pointers. It also queues the DMA
 * buffer back to the DMA system.
 *
 * @param   arg   driver private data
 * @param   error any DMA error
...
static int mxcuart_initdma(dma_info * d_info, uart_mxc_port * umxc)
{
	...
	umxc->rx_dmamap = kmalloc(rxbufs * sizeof(mxc_uart_rxdmamap),
			GFP_KERNEL);
	if (!umxc->rx_dmamap) {
		ret = -1;
 ...
 * the port structures mask registers to indicate the types of events the user
 ...
 * application while setting the UART port information matches what is stored in
 * the define \b PORT_MXC found in the header file include/linux/serial_core.h
 *
 * @param   port   the port structure for the UART passed in by the core driver
