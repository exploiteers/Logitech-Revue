...
asmlinkage off_t sys_lseek(unsigned int fd, off_t offset, unsigned int origin)
{
	...
	MARK(fs_lseek, "%u off_t %ld %u", fd, offset, origin);
			   ...
			   unsigned int origin)
{
	...
	MARK(fs_llseek, "%u loff_t %lld %u", fd, offset, origin);
...
asmlinkage ssize_t sys_read(unsigned int fd, char __user * buf, size_t count)
{
	...
	if (file) {
		...
		MARK(fs_read_ret, "%u %zd __user %p", fd, ret, buf);
...
asmlinkage ssize_t sys_write(unsigned int fd, const char __user * buf, size_t count)
{
	...
	if (file) {
		...
		MARK(fs_write_ret, "%u %zd __user %p", fd, ret, buf);
		file_pos_write(file, pos);
		fput_light(file, fput_needed);
	}
			     ...
			     size_t count, loff_t pos)
{
	...
	if (file) {
		...
		if (file->f_mode & FMODE_PREAD) {
			MARK(fs_pread64, "%u %zu loff_t %lld", fd, count, pos);
			ret = vfs_read(file, buf, count, &pos);
		}
	...
	if (file) {
		ret = -ESPIPE;
 		if (file->f_mode & FMODE_PWRITE) {
			MARK(fs_pwrite64, "%u %zu loff_t %lld", fd, count, pos);
			ret = vfs_write(file, buf, count, &pos);
		}
		fput_light(file, fput_needed);
	}
