...
int neigh_add(struct sk_buff *skb, struct nlmsghdr *nlh, void *arg)
{
	...
	for (tbl = neigh_tables; tbl; tbl = tbl->next) {
		...
		if (ndm->ndm_flags & NTF_PROXY) {
			...
			pn = pneigh_lookup(tbl, RTA_DATA(dst_attr), dev, 1);
			if (pn) {
 ...
 */
static int pneigh_fill_info(struct neigh_table *tbl, struct sk_buff *skb,
			    struct pneigh_entry *pn, u32 pid, u32 seq, int event)
{
	int locked = 0;
	...
	RTA_PUT(skb, NDA_DST, tbl->key_len, pn->key);
	read_unlock_bh(&tbl->lock);
	locked		 = 0;
	ndm->ndm_state	 = 0;
			     ...
			     struct netlink_callback *cb)
{
	...
	for (h = 0; h <= PNEIGH_HASHMASK; h++) {
		...
		if (h > s_h)
			s_idx = 0;
		read_lock_bh(&tbl->lock);
		for (pn = tbl->phash_buckets[h], idx = 0;
		     pn;
		     pn = pn->next, idx++) {
			if (idx < s_idx)
				continue;
			if (pneigh_fill_info(tbl, skb, pn,
					     NETLINK_CB(cb->skb).pid,
					     cb->nlh->nlmsg_seq,
					     RTM_NEWNEIGH) <= 0) {
				read_unlock_bh(&tbl->lock);
