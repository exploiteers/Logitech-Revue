			     ...
			     struct rtattr **xfrma)
{
	...
	case IPPROTO_ROUTING:
		...
		/*
		 * SPI must be 0 until assigning by kernel.
		 */
		if (p->id.spi) {
...
static int xfrm_add_acquire(struct sk_buff *skb, struct nlmsghdr *nlh, void **xfrma)
{
	...
	for (i = 0; i < xp->xfrm_nr; i++, ut++) {
		...
		t->ealgos = ua->ealgos;
		t->calgos = ua->calgos;
		err = km_query(x, t, xp, XFRM_POLICY_OUT);
	}
...
static int xfrm_do_migrate(struct sk_buff *skb, struct nlmsghdr *nlh,
			   void **xfrma)
{
	struct xfrm_user_migrate *m = NLMSG_DATA(nlh);
	struct rtattr *rt = xfrma[XFRMA_POLICY-1];
	struct xfrm_userpolicy_id *pol = NULL;
	int err = -EINVAL;
	...
	if (rt) {
		if ((rt->rta_len - sizeof(*rt)) < sizeof(*pol)) {
			err = -EINVAL;
			goto error;
		}

		pol = RTA_DATA(rt);
		err = verify_policy_dir(pol->dir);
		if (err) {
			XFRM_DBG("%s: invalid dir = %d\n", __FUNCTION__, pol->dir);
			goto error;
		}
	}
	if (!pol) {
...
#ifdef CONFIG_XFRM_MIGRATE
	[XFRM_MSG_MIGRATE     - XFRM_MSG_BASE] = XMSGSIZE(xfrm_user_migrate),
#endif
#ifdef CONFIG_XFRM_ENHANCEMENT
...
#ifdef CONFIG_XFRM_MIGRATE
static int build_migrate(struct sk_buff *skb, struct xfrm_user_migrate *m,
			 struct xfrm_userpolicy_id *id)
{
	...
	nlh = NLMSG_PUT(skb, 0, 0, XFRM_MSG_REPORT, sizeof(*um));
	um = NLMSG_DATA(nlh);
	nlh->nlmsg_flags = 0;
	...
	if (id)
		RTA_PUT(skb, XFRMA_POLICY, sizeof(*id), id);
...
static int xfrm_send_migrate(struct xfrm_user_migrate *m,
			     struct xfrm_userpolicy_id *id)
{
	struct sk_buff *skb;
	size_t len;

	len = RTA_SPACE(sizeof(struct xfrm_userpolicy_id));
	len += NLMSG_SPACE(sizeof(struct xfrm_user_migrate));
	skb = alloc_skb(len, GFP_ATOMIC);
	if (skb == NULL)
		return -ENOMEM;

	if (build_migrate(skb, m, id) < 0)
		BUG();

	NETLINK_CB(skb).dst_group = XFRMNLGRP_MIGRATE;
	return netlink_broadcast(xfrm_nl, skb, 0, XFRMNLGRP_MIGRATE, GFP_ATOMIC);
}
#endif

#ifdef CONFIG_XFRM_ENHANCEMENT
static int build_report(struct sk_buff *skb, __u8 dir, __u8 proto,
			struct xfrm_selector *sel, xfrm_address_t *addr)
{
	...
	XFRM_DBG("%s: dir = %d,\n", __FUNCTION__, dir);
	XFRM_DBG("    ifindex = %d,\n", sel->ifindex);
	XFRM_DBG("    dst = %s,\n", XFRMSTRADDR(sel->daddr, sel->family));
	XFRM_DBG("    src = %s,\n", XFRMSTRADDR(sel->saddr, sel->family));
	if (addr)
		XFRM_DBG("    coa = %s,\n", XFRMSTRADDR(*addr, sel->family));
	XFRM_DBG("    flow = %u, xfrm = %u\n", sel->proto, proto);
...
static int xfrm_send_report(__u8 dir, __u8 proto, struct xfrm_selector *sel,
			    xfrm_address_t *addr)
{
	struct sk_buff *skb;
	size_t len;
	...
	if (build_report(skb, dir, proto, sel, addr) < 0)
		BUG();
