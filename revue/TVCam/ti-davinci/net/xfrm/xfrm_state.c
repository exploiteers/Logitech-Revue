...
int km_query(struct xfrm_state *x, struct xfrm_tmpl *t, struct xfrm_policy *pol, int dir);
		...
		unsigned short family)
{
...
#if 0
	XFRM_DBG("%s: tmpl search key :\n", __FUNCTION__);
	XFRM_DBG("     reqid = %d, mode = %d, proto = %d,\n", tmpl->reqid,
		 tmpl->mode, tmpl->id.proto);
	XFRM_DBG("     daddr = %s,\n", XFRMSTRADDR(daddr, family));
	XFRM_DBG("     saddr = %s\n", XFRMSTRADDR(saddr, family));
#endif
	spin_lock_bh(&xfrm_state_lock);
	...
	if (!x && !error && !acquire_in_progress) {
		...
		if (km_query(x, tmpl, pol, XFRM_POLICY_OUT) == 0) {
...
static void __xfrm_state_insert(struct xfrm_state *x)
{
	...
	if (xfrm_id_proto_match(x->id.proto, IPSEC_PROTO_ANY) || x->id.spi) {
...
static void __xfrm_state_bump_genids(struct xfrm_state *xnew)
{
	...
	h = xfrm_dst_hash(&xnew->id.daddr, family);
	list_for_each_entry(x, xfrm_state_bydst+h, bydst) {
		if (x->props.family == family &&
			x->props.reqid  == reqid &&
			 ...
			 void *arg, int (*filter)(struct xfrm_state *, void *))
{
	...
	int err = 0;
	int ret;
	u32 h = xfrm_dst_hash(daddr, family);
	...
	list_for_each_entry(x, xfrm_state_bydst+h, bydst) {
		...
		e = kmalloc(sizeof(struct xfrm_state_bulk_entry), GFP_ATOMIC);
		if (!e) {
			err = -ENOMEM;
			goto out;
		}
...
static struct xfrm_algo *xfrm_algo_solidclone(struct xfrm_algo *old)
{
	struct xfrm_algo *a;
...
struct xfrm_state *xfrm_state_solidclone(struct xfrm_state *old, int *errp)
{
	...
	if (old->aalg) {
		x->aalg = xfrm_algo_solidclone(old->aalg);
		if (!x->aalg)
			goto error;
	}
	...
	if (old->ealg) {
		x->ealg = xfrm_algo_solidclone(old->ealg);
		if (!x->ealg)
			goto error;
	}
	...
	if (old->calg) {
		x->calg = xfrm_algo_solidclone(old->calg);
		if (!x->calg)
			goto error;
	}
...
EXPORT_SYMBOL(xfrm_state_solidclone);
#endif
...
#ifdef CONFIG_XFRM_MIGRATE
int km_migrate(struct xfrm_user_migrate *m, struct xfrm_userpolicy_id *id)
{
	int err = -EINVAL;
	int ret;
	struct xfrm_mgr *km;
...
#ifdef CONFIG_XFRM_ENHANCEMENT
int km_report(__u8 dir, __u8 proto, struct xfrm_selector *sel, xfrm_address_t *addr)
{
	int err = -EINVAL;
	int ret;
	struct xfrm_mgr *km;
	...
	list_for_each_entry(km, &xfrm_km_list, list) {
		if (km->report) {
			ret = km->report(dir, proto, sel, addr);
			if (!ret)
				err = ret;
		}
	}
...
#ifdef CONFIG_XFRM_ENHANCEMENT
int xfrm_ro_acq(struct flowi *fl, struct xfrm_tmpl *t, struct xfrm_policy *xp, unsigned short family)
{
	xfrm_address_t *daddr = xfrm_flowi_daddr(fl, family);
	xfrm_address_t *saddr = xfrm_flowi_saddr(fl, family);
	struct xfrm_state x;

	xfrm_init_tempsel(&x, fl, t, daddr, saddr, family);
	return km_query(&x, t, xp, XFRM_POLICY_IN);
}
#endif
