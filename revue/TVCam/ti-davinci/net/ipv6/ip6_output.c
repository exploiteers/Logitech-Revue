...
static inline int ip6_dst_saddr_check(struct sock *sk,
                                      struct flowi *fl,
                                      struct ipv6_pinfo *np,
                                      struct rt6_info *rt)
{
#ifdef CONFIG_IPV6_SUBTREES
        /* no old source address, do a new route lookup to get a new one */
        return ((rt->rt6i_src.plen == 128 &&
                 ipv6_addr_equal(&fl->fl6_src, &rt->rt6i_src.addr)) ||
                (np->saddr_cache != NULL &&
                 ipv6_addr_equal(&fl->fl6_src, np->saddr_cache)));
#else
        return (!ipv6_addr_any(&fl->fl6_src) || (sk && inet_sk(sk)->hdrincl) ||
                ipv6_get_saddr(&rt->u.dst, &fl->fl6_dst, &fl->fl6_src) >= 0);
					  ...
					  struct flowi *fl)
{
	 ...
	 * 3. If source address based routing is used
	 *    do the same checks as for the destination
	 *    address.
	 */
...
#endif
	    !ip6_dst_saddr_check(sk, fl, np, rt) ||
