...
static inline void fib6_new_sernum(struct fib6_node *fn)
{
	u32 n = ++rt_sernum;
	if ((__s32)n <= 0)
		rt_sernum = n = 1;
...
#ifdef CONFIG_IPV6_SUBTREES
struct fib6_node * fib6_subtree_lookup(struct fib6_node *parent,
				       struct in6_addr *saddr)
{
	struct lookup_args args;
	struct fib6_node *fn;

	args.offset = offsetof(struct rt6_info, rt6i_src);
	args.addr = saddr;

	fn = fib6_lookup_1(parent->subtree, &args);
...
void rt6_ifa_del(struct inet6_ifaddr *ifa)
{
	...
	for (i = FIB6_TABLE_MIN; i <= FIB6_TABLE_MAX; i++) {
		struct fib6_table *table = fib6_get_table(i);
		if (table) {
			write_lock(&table->tb6_lock);
			fib6_clean_tree(&table->tb6_root, fib6_ifa_del, 0, ifa);
			write_unlock(&table->tb6_lock);
		}
	}
}
