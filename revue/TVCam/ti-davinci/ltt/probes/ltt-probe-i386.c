...
#include <ltt/ltt-facility-kernel_arch_i386.h>
#include <ltt/ltt-stack.h>

#define KERNEL_TRAP_ENTRY_FORMAT "%d struct pt_regs %p"
...
void probe_kernel_syscall_entry(const char *format, ...)
{
	...
	do_trace_kernel_stack_dump(regs);
	do_trace_process_stack_dump(regs);
...
void probe_mm_handle_fault_entry(const char *format, ...)
{
	...
	/* Call tracer */
	if(in_irq())
		trace_kernel_trap_entry(trap_id, (void*)eip);
